...
export function DashboardPage({ raids, debugInfo, decoderStats, onRaidSelect }: DashboardPageProps) {
  const [filters, setFilters] = useState<RaidFilters>(defaultFilters);

  const filteredRaids = useMemo(() => {
    return filterRaids(raids, filters).sort(
    ...
    <div className="space-y-4">
      <section className="panel p-3">
        <div className="flex flex-col gap-3 xl:flex-row xl:items-end xl:justify-between">
          <div>
            <p className="text-[11px] uppercase text-abi-muted">Local ABInfinite.log</p>
            <h1 className="mt-1 text-xl font-semibold text-abi-text">Dashboard</h1>
          </div>
          <div className="grid grid-cols-3 gap-2 text-right text-xs sm:min-w-[360px]">
            ...
            <div className="border border-abi-line bg-abi-black px-3 py-2">
              ...
              <p className="font-mono text-lg text-abi-amber">{debugInfo ? "Live" : "Idle"}</p>
            </div>
          </div>
        </div>
      </section>
      ...
      <SectionPanel title="최근 Raid 목록" eyebrow="Raids">
        <RaidTable raids={filteredRaids} onRaidSelect={onRaidSelect} />
      </SectionPanel>
